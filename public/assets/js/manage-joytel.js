...
function buildExistingRateMap() {
    const rates = {};
    const rows = Array.isArray(window.additionalPrices) ? window.additionalPrices : [];
...
function mergeExistingRates(button) {
    ...
    if (button) {
        try {
            const buttonRates = JSON.parse(button.getAttribute('data-existing-rates') || '{}');
...
function syncExistingRateCache(updates) {
    ...
    const cache = new Map(window.additionalPrices.map((row) => [row.product_code, { ...row }]));

    updates.forEach((update) => {
        if (!update?.product_code) return;

        const existing = cache.get(update.product_code) || { product_code: update.product_code };
    ...
    if (currentPriceTrigger) {
        currentPriceTrigger.setAttribute('data-existing-rates', JSON.stringify(buildExistingRateMap()));
    }
}
...
if (manageStatusModal) {
    manageStatusModal.addEventListener('show.bs.modal', function (event) {
        ...
        const originalBtn = manageStatusModal.querySelector('button.btn-primary');
...
if (managePriceModal) {
    managePriceModal.addEventListener('show.bs.modal', function (event) {
        ...
        planData.forEach((item, index) => {
            ...
            const totalValue = sellingRate > 0 ? Math.round(portalPrice * sellingRate) : 0;
            const profitValue = sellingRate > 0 ? Math.round(totalValue - exchangeRateAuto) : 0;
        ...
        tbody.querySelectorAll('.selling-rate').forEach((input) => {
            input.addEventListener('input', function () {
                ...
                const portalPrice = toNumber(row.querySelector('.portal-price')?.innerText);
                ...
                row.querySelector('.total-label').innerText = totalValue.toLocaleString();
                row.querySelector('.profit-label').innerText = profitValue.toLocaleString();
            });
        });
    });
}
...
if (priceUpdateBtn) {
    priceUpdateBtn.addEventListener('click', function () {
        const tbody = managePriceModal.querySelector('#price-invoice-items');
        const joytelId = currentPriceTrigger?.getAttribute('data-joytel-id') || managePriceModal.getAttribute('data-current-joytel-id');
        const updates = [];
        const joytelType = getJoytelType(currentPriceTrigger);
        ...
        fetch('/joytel/update-price', {
            ...
            .then((data) => {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error!',
                    text: (data && data.message) || 'Failed to update exchange rates.',
                });
            })
            .catch((error) => {
                console.error(error);
...
document.getElementById('confirmDeleteBtn')?.addEventListener('click', function () {
    ...
    fetch(`/joytel/delete-esim/${selectedSimId}`, {
        ...
        .then(() => {
            const modal = bootstrap.Modal.getInstance(document.getElementById('sim-delete'));
            if (modal) modal.hide();

            document.querySelector(`[data-id="${selectedSimId}"]`)?.closest('tr')?.remove();
