...
@section('title', 'Joytel Physical sim')
@section('content')
    <style>
        .joytel-physical-page-title,
        .joytel-physical-breadcrumb-current {
            color: #111827;
        }

        .joytel-physical-breadcrumb-link {
            color: #4b5563;
        }

        .joytel-physical-breadcrumb-link:hover {
            color: #1f2937;
        }

        html[data-bs-theme="dark"] .joytel-physical-page-title,
        html[data-bs-theme="dark"] .joytel-physical-breadcrumb-current {
            color: #e5edf9;
        }

        html[data-bs-theme="dark"] .joytel-physical-breadcrumb-link {
            color: #9fb1cc;
        }

        html[data-bs-theme="dark"] .joytel-physical-breadcrumb-link:hover {
            color: #dbe7ff;
        }
    </style>
                ...
                <div data-table data-table-rows-per-page="8" class="card">
                    <div class="card-header border-light justify-content-between">
                        ...
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <!-- Date Range Filter -->
                            <div class="app-search">
                                <select id="product_status" data-table-filter="product-status"
                    ...
                    <div class="modal fade" id="export_csv" tabindex="-1" aria-labelledby="exampleModalLabel"
                        ...
                        <div class="modal-dialog">
                            <form action="{{ route('joytel.import.recharge') }}" method="post"
                                ...
                                @if ($sim_lists->isNotEmpty())
                                    @foreach ($sim_lists as $index => $recharge)
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <h5 class="text-nowrap fs-base mb-0 lh-base">{{ $recharge->product_name }}
                                                </h5>
                                            </td>
                                            <td>{{ $recharge->provider }}</td>

                                            <td data-column="product-status"
                                                class="{{ $recharge->status == 1 ? 'text-success' : 'text-danger' }} fw-semibold">
                                                <i class="ti ti-point-filled fs-sm"></i>
                                                {{ $recharge->status == 1 ? 'Enable' : 'Disable' }}
                                            </td>
                                            <td>
