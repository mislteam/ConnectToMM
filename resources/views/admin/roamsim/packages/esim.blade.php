                ...
                <div class="row">
                    <div class="col-12">
                        <div data-table data-table-rows-per-page="8" class="card">
                            <div class="card-header border-light justify-content-between">
                                ...
                                <div class="d-flex gap-2">
                                    <div class="app-search">
                                        <input data-table-search type="search" class="form-control" placeholder="Search Plan Name...">
                                        ...
                                        @foreach ($packages as $index => $pkg)
                                            ...
                                            @if(!empty($plans->packages))
                                             <tr>
                                                ...
                                                <td class="text-success fw-semibold"><i class="ti ti-point-filled fs-sm"></i> Enable
                                                </td>
                                                <td>
                                                    <div class="d-flex justify-content-center gap-1">
                                                        <div class="btn-group">
                                                           ...
                                                           <div class="dropdown-menu">
                                                                ...
                                                                <button type="button" class="dropdown-item" data-bs-toggle="modal" data-bs-target="#manage-price-{{ $index }}">
                                                                    <i class="ti ti-currency-dollar fs-lg"></i> Manage Price
                                                                </button>
                                                                <button type="button" class="dropdown-item" data-bs-toggle="modal" data-bs-target="#manage-status-{{ $index }}">
                                                                    <i class="ti ti-box fs-lg"></i> Manage Status
                                                                </button>
                                                            </div>
                                                        </div>
                                                        <a href="{{ route('roamEsimEdit', ['skuid' => $pkg['sku_id']]) }}" class="btn btn-light btn-icon btn-sm rounded-circle"><i class="ti ti-edit fs-lg"></i></a>
                            ...
                            @foreach ($packages as $index => $pkg)
                            <div class="modal fade" id="manage-price-{{ $index }}" tabindex="-1" role="dialog" aria-labelledby="managePrice{{ $index }}" aria-hidden="true">
                                                        ...
                                                        @if(!empty($plans->packages))
                                                            @foreach ($plans->packages as $innerIndex => $plan)
                                                                @php
                                                                    ...
                                                                    $savedRate = App\Models\PriceList::where('product_code', $plan['priceid'])->where('dp_status', 0)->first();
                                                                ...
                                                                <tbody id="invoice-items">
                                                                        <tr>
                                                                            ...
                                                                            <td>
                                                                                <input type="number" class="form-control exchange-input" name="plans[{{ $innerIndex }}][exchange_rate]" value="{{ $exchange_rate }}" step="0.01">
                                                                            </td>
                            ...
                            @foreach ($packages as $index => $pkg)
                            <div class="modal fade" id="manage-status-{{ $index }}" tabindex="-1" role="dialog" aria-labelledby="manageStatusLabel{{ $index }}" aria-hidden="true">
                                <div class="modal-dialog modal-lg modal-dialog-scrollable">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                                            ...
                                                            <tbody id="invoice-items">
                                                                <tr>
                                                                    ...
                                                                    <td>
                                                                        ...
                                                                        <form action="{{ route('roam.updatePackageStatus') }}" method="POST">
                                                                            @csrf
                                                                            <input type="hidden" name="plans[{{ $index }}][sku_id]" value="{{ $pkg->sku_id }}">
                                                                            <input type="hidden" name="plans[{{ $index }}][priceid]" value="{{ $plan['priceid'] }}">
                                                                            <input type="hidden" name="plans[{{ $index }}][plan_name]" value="{{ $plan['flows'] ?? '' }} {{ $plan['unit'] ?? '' }}">
