<div class="sidenav-menu">
    ...
    <div class="scrollbar" data-simplebar>
        ...
        <ul class="side-nav">
            ...
            @if (auth()->user()->can('order.menu'))
                <li class="side-nav-item {{ $isOrderActive ? 'active' : '' }}">
                    ...
                    <div class="collapse {{ $isOrderActive ? 'show' : '' }}" id="sidebarOrders">
                        <ul class="sub-menu">
                            <li
                                class="side-nav-item {{ request()->routeIs('order.index', 'order.show') ? 'active' : '' }}">
                                <a href="{{ route('order.index') }}" class="side-nav-link">
                                    <span class="menu-text">{{ $settings['roam_title']->value ?? 'FiROAM' }}
                                        Orders</span>
                                </a>
                            </li>
                            <li
                                class="side-nav-item {{ request()->routeIs('order.joytel', 'order.joytel.show') ? 'active' : '' }}">
                                <a href="{{ route('order.joytel') }}" class="side-nav-link">
                                    <span class="menu-text">{{ $settings['joytel_title']->value ?? 'JOYTEL' }}
                                        Orders</span>
                                </a>
                            </li>
                        </ul>
            ...
            <x-page-nav-group :active="$isJoyActive" :menu-text="$settings['joytel_title']->value ?? 'Joytel'" :any-permission="$joytelPermissions" side-link-name="sidebarjoytel"
                icon="ti-device-sim">
                ...
                <x-page-nav permission="joytel.coupon.menu" :url="route('joytel.coupon.index')" title="Coupon" :active="request()->routeIs('joytel.coupon.*')" />
            </x-page-nav-group>

            <x-page-nav-group :active="$isRoamActive" :menu-text="$settings['roam_title']->value ?? 'ROAM'" :any-permission="$roamPermissions" side-link-name="sidebarRoam"
                icon="ti-device-sim">
            ...
            <x-page-nav-group :any-permission="$blogPermissions" menu-text="Blog" :active="request()->routeIs('blog.*', 'blog.category.*')" side-link-name="sidebarBlog"
                ...
                <x-page-nav permission="blog.category.menu" :url="route('blog.category.index')" title="Categories" :active="request()->routeIs('blog.category.*')" />
            </x-page-nav-group>

            <x-page-nav-group :active="request()->is('setting/*') || request()->routeIs('currency.*', 'permission.*')" menu-text="Setting" side-link-name="sidebarSetting" icon="ti-settings"
                :any-permission="$generalPermissions">
            ...
            <x-page-nav permission="payment.menu" :url="route('admin.payment.index')" title="Payment Setting" :active="request()->routeIs('admin.payment.*')"
            ...
            <x-page-nav-group :active="request()->routeIs('page.*', 'footer.important.*', 'footer.support.*', 'footer.contact.*')" menu-text="Page" side-link-name="sidebarPage" icon="ti-clipboard"
                ...
                <x-page-nav-group :active="$isFooterActive" menu-text="Footer" side-link-name="footerPage"
                    ...
                    <x-page-nav permission="page.menu" :url="route('footer.support.index')" title="Support" :active="request()->routeIs('footer.support.*')" />
                    <x-page-nav permission="page.menu" :url="route('footer.contact.index')" title="Get In Touch" :active="request()->routeIs('footer.contact.*')" />
                </x-page-nav-group>

                <x-page-nav permission="page.menu" :url="route('page.common.index')" title="Common Sections" :active="$isCommonActive" />
                <x-page-nav permission="page.menu" :url="route('page.refunds.index')" title="Refunds Policy" :active="request()->routeIs('page.refunds.*')" />
            </x-page-nav-group>
        </ul>
    </div>
</div>
