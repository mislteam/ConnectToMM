...
@section('content')
                ...
                <div class="col-lg-6 col-md-6">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-8 col-md-8 col-sm-12 col-12">
                            <div class="content">
                                <label class="text">{{ $settings['roam_title']->value ?? 'Roam' }}</label>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        ...
                        <div class="col-lg-8 col-md-8 col-sm-12 col-12">
                            <div class="content">
                                @php
                                    ...
                                    $operators = collect(data_get($firstPackage, 'networkDtoList', []))
                                        ->pluck('operator')
                                        ->unique()
                                        ->join(', ');
                                @endphp
                            ...
                            <div class="form-group">
                                ...
                                <div id="dataPlan" class="btn-group btn-group-toggle d-flex flex-wrap"
                                    ...
                                    @foreach ($validPackages as $plan)
                                        @php
                                            $isUnlimited = stripos($plan['showName'] ?? '', 'Unlimited') !== false;
                                            $flow = $isUnlimited
                                                ? 'Unlimited'
                                                : trim(($plan['flows'] ?? '') . ' ' . ($plan['unit'] ?? ''));
                                            $portalPrice = round(
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const daypassDaysInput = document.getElementById('daypass_days');
            ...
            function getSelectedDataInput() {
                return dataBox.querySelector('input[name="sdata"]:checked');
            }
            ...
            function syncHiddenFields(selectedData = null) {
                if (!selectedData) {
                    ...
                    if (daypassDaysInput) daypassDaysInput.value = '';
                    return;
                }

                const unitPrice = parseFloat(selectedData.dataset.price || 0) || 0;
                const qty = parseInt(qtyInput.value, 10) || 1;
                const totalPrice = Math.round(unitPrice * qty);
                const selectedDayInput = service_day_box.querySelector('input[name="sday"]:checked');

                if (displayPriceInput) displayPriceInput.value = totalPrice;
                if (unitPriceInput) unitPriceInput.value = unitPrice;
                if (totalPriceInput) totalPriceInput.value = totalPrice;
                if (apiCodeInput) apiCodeInput.value = selectedData.dataset.apicode || '';
                if (priceIdInput) priceIdInput.value = selectedData.dataset.priceId || '';
                if (planNameInput) planNameInput.value = selectedData.dataset.planName || '';
                if (daypassDaysInput) {
                    daypassDaysInput.value = selectedDayInput ? (selectedDayInput.dataset.day || selectedDayInput
                        .value) : '';
                }
            }
            ...
            function renderDataPlans(plans, selectedDay, selectedType, preferredFlowKey = null) {
                ...
                validPlans.forEach((plan, index) => {
                    ...
                    const priceListItem = getPriceListItem(plan);
                    ...
                    const dataLabel = String(plan.showName || '').toLowerCase().includes('unlimited') ?
                        'Unlimited' :
                        `${plan.flows} ${plan.unit}`;
                    const shouldSelect = index === 0;
                    const label = document.createElement('label');
                    label.className =
                        `btn btn-outline-secondary m-1 rounded ${shouldSelect ? 'active' : ''}`;
                    label.innerHTML = `
                <input type="radio" name="sdata" value="${dataLabel}"
                    ...
                    data-apicode="${apiCode}"
                    data-price-id="${priceListItem ? priceListItem.id : ''}"
                    data-plan-name="${plan.showName || dataLabel}"
            ...
            function updatePriceDisplay() {
                ...
                if (selectedData) {
                    ...
                    ===
                    =
                    const total = Math.round((parseFloat(selectedData.dataset.price || 0) || 0) * qty); >>>
                    >>>
                    > Stashed changes
            ...
            service_day_box.addEventListener('click', function(e) {
                ...
                service_day_box.querySelectorAll('label').forEach(item => item.classList.remove('active'));
                label.classList.add('active');
                input.checked = true;
