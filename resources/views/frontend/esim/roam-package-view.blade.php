...
@section('content')
 ...
 <!-- Sub-Banner -->
        <div class="sub-banner">
            ...
            <div class="box">
                <span class="mb-0 text-size-16">Our Service</span><span class="mb-0 text-size-16 dash">-</span><span class="mb-0 text-size-16">E-SIM</span><span class="mb-0 text-size-16 dash">-</span><span class="mb-0 text-size-16 box_span">Roam</span>
            </div>
        </div>
        <!--Services section-->
        <section class="service-section">
            <div class="container">
                <div class="row mb-5">
                     @php
                         $pkg = App\Models\Roam::where('sku_id', $sku->sku_id)->first();
                        ...
                        <div id="productGlleryIndicators" class="" data-ride="carousel">
                          ...
                          <div class="carousel-inner">
                            <div class="carousel-item active">
                              <img class="d-block w-100" src="{{  file_exists(public_path('storage/upload/roam/' . $pkg->image)) ? asset('storage/upload/roam/' . $pkg->image) : asset($pkg->image ?? 'assets/images/package.jpg') }}">
                            </div>
                    ...
                    <div class="col-lg-6 col-md-6">
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-8 col-md-8 col-sm-12 col-12">
                                <div class="content">
                                    @php
                                        $operators = collect($activePackages[0]['networkDtoList'])
                                            ->pluck('operator')
                                            ->unique()
                                            ->join(', ');
                                    @endphp
                        ...
                        <form class="form-design">
                           <div class="form-group">
                            ...
                            <div id="trafficType" class="btn-group btn-group-toggle d-flex flex-wrap" data-toggle="buttons">
                            ...
                            <div class="form-group">
                                ...
                                <div id="serviceDay" class="btn-group btn-group-toggle d-flex flex-wrap" data-toggle="buttons">
                                    @php
                                    ...
                                    @foreach ($uniqueDays as $day)
                                        <label class="btn btn-outline-secondary m-1 {{ $loop->first ? 'active' : '' }}">
                                            <input type="radio" name="sday" value="{{ $day }} Days" data-day="{{ $day }}" {{ $loop->first ? 'checked' : '' }}>
                                            {{ $day }} Days
                                        </label>
                                    @endforeach
                                </div>
                           ...
                           <div class="form-group">
                                ...
                                <div id="dataPlan" class="btn-group btn-group-toggle d-flex flex-wrap" data-toggle="buttons">
                                    @foreach ($activePackages as $plan)
                                        ...
                                        <label class="btn btn-outline-secondary m-1 rounded d-none"
                                            ...
                                            data-sku="{{ $roam['sku_id'] }}">
                                            <input type="radio" name="sdata" value="{{ $flow }}"> {{ $flow }}
                                        </label>
                                    @endforeach
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                              ...
                              <div class="input-group quantity-wrapper">
                                ...
                                <input type="number" id="qty" class="form-control text-center" value="1" min="1" max="100">
                    ...
                    <div class="row" >
                        @forelse ($randomSkus as $package)
                            @php
                                ...
                                $itemPriceList = App\Models\PriceList::where('plan', $package->sku_id)->where('dp_status',0)->first();
                                ...
                                if ($itemRoam && !empty($itemRoam->packages)) {
                                    ...
                                    $lowestPrice = collect($itemRoam->packages)
                                        ...
                                        ->map(function ($pkg) use ($priceMap) {
                                            if (!isset($priceMap[$pkg['priceid']])) return null;
                        ...
                        <div class="col-lg-4 col-md-4 col-sm-12 col-12">
                            <div class="service-box">
                                ...
                                <div class="content">
                                    ...
                                    @endif
                                    <a href="{{ route('esim.roampackageview', ['id' => $package->sku_id]) }}" class="more">View Offer</a>
                                </div>
                            </div>
                        </div>
                         @empty
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!hasValidPlans) {
        ...
        elements.forEach(id => { if(document.getElementById(id)) document.getElementById(id).innerHTML = ''; });
        return;
    }
    ...
    function filterTrafficTypes() {
        ...
        allPackages.forEach(pkg => {
            ...
            if (isProductValid(pkg.priceid)) {
                ...
                console.warn(`PriceID ${pkg.priceid} (${pkg.showName}) not found in price_lists table.`);
            }
        });
    ...
    service_day_box.addEventListener('click', function(e) {
        ...
        const type = document.querySelector('#trafficType input[name="tType"]:checked').value.toLowerCase();
