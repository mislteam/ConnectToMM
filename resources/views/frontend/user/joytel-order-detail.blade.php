...
@section('content')
                                        ...
                                        <div class="mt-4">
                                            ...
                                            <div class="sim-details-list">
                                                <div
                                                    ...
                                                    <div>Code</div>
                                                    <div class="sim-details-pdf">QR</div>
                                                ...
                                                @foreach ($order->items as $item)
                                                    ...
                                                    <div
                                                        ...
                                                        <div class="sim-details-pdf">
                                                            ...
                                                            @if ($qrImage)
                                                                <button type="button" class="btn btn-link p-0 pdf-link"
                                                                    data-bs-toggle="modal" data-bs-target="#joytelQrModal"
                                                                    data-qr-src="{{ asset('storage/' . $qrImage) }}"
                                                                    data-qr-title="{{ e($productName) }}">
                                                                    View QR Code
                                                                </button>
                                                            @else
                                                                -
                                                        ...
                                                        <div class="sim-details-pdf">
                                                            <button type="button"
                                                                ...
                                                                data-bs-toggle="modal" data-bs-target="#joytelUsageModal"
                                                                data-manual="0" data-outer-order-id="{{ $outer_order_id }}"
    ...
    @if (($provider ?? '') === 'joytel')
        <div class="modal fade" id="joytelQrModal" tabindex="-1" aria-labelledby="joytelQrModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="joytelQrModalLabel">QR Code</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body text-center">
                        <img src="" alt="Joytel QR Code" class="qr-lightbox-image d-none" id="joytelQrImage">
                    </div>
                </div>
            </div>
        </div>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                var joytelQrModal = document.getElementById('joytelQrModal');
                var joytelQrImage = document.getElementById('joytelQrImage');
                var joytelQrModalLabel = document.getElementById('joytelQrModalLabel');

                if (joytelQrModal && joytelQrImage) {
                    joytelQrModal.addEventListener('show.bs.modal', function(event) {
                        var button = event.relatedTarget;
                        var src = button ? button.getAttribute('data-qr-src') : '';
                        var title = button ? button.getAttribute('data-qr-title') : '';

                        joytelQrImage.src = src || '';
                        joytelQrImage.classList.toggle('d-none', !src);
                        if (joytelQrModalLabel) {
                            joytelQrModalLabel.textContent = 'QR Code';
                        }
                    });

                    joytelQrModal.addEventListener('hidden.bs.modal', function() {
                        joytelQrImage.src = '';
                        joytelQrImage.classList.add('d-none');
                        if (joytelQrModalLabel) {
                            joytelQrModalLabel.textContent = 'QR Code';
                        }
                    });
